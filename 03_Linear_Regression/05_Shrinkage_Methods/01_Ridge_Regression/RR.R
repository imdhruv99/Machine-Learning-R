# importing data
df <- read.csv("D:/Programming/Machine Learning/Machine Learning with Python and R/R/03_Linear_Regression/05_Shrinkage_Methods/01_Ridge_Regression/House_Price.csv",
               header = TRUE)
...
# assigning True value to Training Set and False value to Testing set
training_set = subset(df, split == TRUE)
testing_set = subset(df, split == FALSE)

# Running Linear Model on testing Dataset
lr_p = lm(price~., data=training_set)

# Finding Mean Square Error [MSE]
# so, first we need to predict the value of price
# we use predict function which takes 2 parameters. one is model and other is Data
train_p = predict(lr_p, training_set)
test_p = predict(lr_p, testing_set)

# MSE is Average of difference of the Square of the predicted and actual values
mean((training_set$price-train_p)^2)
# 20.66
mean((testing_set$price-test_p)^2)
# 33.04
...
# we also need to segregate our data in two dependent very well and independent

# x has all the independent variable
x <- model.matrix(price~., data = df)[,-1]
View(x)
# [,-1] by this we will not get price column

# y has dependent variable
y <- df$price
View(y)

# we need to find out critical lambda
grid = 10^seq(10, -2, length = 100)
grid
# grid has 100 values, it starts with 10^10 and with 10^-2

# running model
rr <- glmnet(x,y, alpha = 0, lambda = grid)
summary(rr)

# we need to find out is critical value of lambda, for which we have the best model
# glmnet have cross validation function
cv_fit <- cv.glmnet(x,y, alpha = 0, lambda = grid)
plot(cv_fit)
# from plot
# we can see this is the graph of log values of lambda vs Mean Squared Error
# Among all we need to find MSE = lowest and corresponding to that we want to find lambda value

# Finding optimum value of lambda
opt_lambda <- cv_fit$lambda.min
opt_lambda

# Finding corresponding R-Squared value of model for optimum lambda
# so first we will find total some of squares
tss <- sum((y-mean(y)) ^ 2)
tss

# now we need to find residual some of squares
# for that we need to find predicted value of y
y_predicted <- predict(rr, s = opt_lambda, newx = x)
